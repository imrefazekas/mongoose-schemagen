...
exports.saveOrUpdate = function( model, prototype, selector, options, callback ){
	if( selector ){
		model.update( selector, prototype, options || { multi: true }, function(err, numAffected) {
			if (err)
				callback( err );
			else if( numAffected === 0 )
				new model( prototype ).saveWithValidation( callback );
			else
				callback( null, prototype);
		} );
	}
	else
		new model( prototype ).saveWithValidation( callback );
...
exports.generate = function(object, validation, schemaoptions, modelOptions){
	...
	model.timeStamped = modelOptions.timeStamped;
	model.creationFunt = modelOptions.creationFunt;

	if( model.timeStamped )
		model.prototype.saveWithTime = function(callback){
			this.timestamp=Date.now();
			if(this.creationFunt)
				this.creationFunt(this);
			this.save(callback);
		};
