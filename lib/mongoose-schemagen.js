...
let innerKeys = [ '_index', '_indexed', '_unique', '_type', '_required' ]
		...
		for ( let key of Object.keys( _prototype ) ) {
			...
			if ( key === '_encrypted' ) continue
				...
				if ( value._type ) {
					...
					if ( value._index )
						_schema[key].index = value._index
					else if ( value._indexed )
						_schema[key].index = true
					else if ( value._unique )
						_schema[key].index = {
							unique: true
						}
...
exports.parseModel = Schema.parseModel
exports.ensureIndexing = Schema.ensureIndexing
exports.modelTypes = Schema.modelTypes
