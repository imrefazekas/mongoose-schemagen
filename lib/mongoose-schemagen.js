...
exports.readModels = function(env, folder, attribute){
	var environment = env || {};
	var files = fs.readdirSync( folder || './' );
	_.each( _.filter( files, function(file){ return file.indexOf('.js', file.length-3) !== -1; } ), function(file, index, list){
		var name = file.substring( 0, file.length-3 );
		environment[ name ] = require( folder + '/' + file )[ attribute || 'dataModel' ];
	} );
	return environment;
};
