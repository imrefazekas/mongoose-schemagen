...
exports.readModels = function(env, folder, recursive){
	...
	_.each( files, function(file, index, list){
		...
		if( isFile && file.indexOf('.js', file.length-3) !== -1 ){
			...
			var newModel = require( folder + '/' + file );
			if( !newModel.transient )
				environment[ name ] = newModel;
		}
	...
	var _GenerateSchema = function( _prototype, _schema ){
		_.each( _prototype, function(value, key, list){
			if( _.isArray( value ) ){
				...
				if( value._type )
					_schema[key] = { type: value._type };
				else if( value.fn || (value.read && value.write) ){
					if( options.allowFunction )
						_schema[key] = { fn: Function };
				} else if( _.keys(value).length === 0  || (_.keys(value).length === 1 && value._observable) )
...
exports.generate = function(mongoose, object, validation, schemaoptions, modelOptions){
	...
	if( model.prototype.timeStamped )
		schema.pre('save', true, function (next, done) {
			...
			if( this.validator ){
				var res = this.validator( this, this.validationRules() );
				if( res && _.keys( res ).length>0 )
					error = new Error( 'Validation error(s) occurred:' + JSON.stringify(res) );
			}
