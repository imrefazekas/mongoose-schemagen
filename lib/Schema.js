...
function convertAttribute ( attribute ) {
	...
	case 'Number':
	case 'Timestamp': return assigner.assign( { _type: Number, _date: attribute.type === 'Timestamp' }, createAttribute( attribute, 0 ) )
...
module.exports = {
	...
	ensureIndexing: function ( aModel, schema ) {
		let index = { }
		for ( let attribute of aModel.attributes ) {
			if ( attribute.type === 'Geo' || attribute.type === 'Array of Geo' ) 
				schema.index( _create( attribute.name, '2dsphere' ), {
					name: 'idx_' + attribute.name
				} )
			else if ( attribute.indexed ) {
				index[ attribute.name ] = attribute.type === 'Number' ? 1 : 'text'
			}
		}
